					...
					<article class="span8 text-center">
						<div>
							<h1 class="inline-block pull-left">
								<a href="/<?=$url_abu?>"><i class="<?=$ico_pag_abu?> ico-titulo"></i></a>
							</h1>	
							<h1 class="inline-block pull-left">
								<a href="/<?=$url_abu?>" class="titulo"><?=$tit_pos_abu?></a> » 
							</h1>
							<div class="inline-block pull-left text-left titulo">
								<div class="dropdown">
									<a href="#" class="dropdown-toggle " data-toggle="dropdown">
										<h1> <?=$tit_pos_pad?><b class="caret"></b></h1>
									</a>
									<ul class="dropdown-menu">
										<?php
											//tipo
											$dato_tip_tour = "SELECT * FROM pagina WHERE per_pagina='$per_pad'";
						...
						<small>
							<dl class="dl-horizontal text-left">
								...
								<?php
									...
									$gen_pag = "SELECT per_tprecio, val_tprecio, ini_tprecio, fin_tprecio FROM tprecio WHERE id_tour='$id_tour' AND vis_tprecio='1' AND  ofe_tprecio='0' ORDER BY per_tprecio";
									...
									?>
									<a class="tooltip-test" data-placement="right" title="Vigencia: <?=$ini_tprecio?> - <?=$fin_tprecio?>" ><small><?=$per_tprecio?> personas  x <?=$val_tprecio?> USD</small></a><br>
								...
								<?php
									...
									//Datos de los Precios de los Tours
									$gen_pag = "SELECT per_tprecio, val_tprecio, ini_tprecio, fin_tprecio FROM tprecio WHERE id_tour='$id_tour' AND vis_tprecio='1' AND  ofe_tprecio='1' ORDER BY per_tprecio";
									$gene_pag = mysql_db_query($dbname, $gen_pag); 
									while ($row = mysql_fetch_array($gene_pag)){ 
							...
							?>
								<div class="accordion-group">
									<div class="accordion-heading">
										<a class="accordion-toggle" data-toggle="collapse" data-parent="#itinerario" href="#itinerario<?=$ord_itinerario?>">
											<?=$tit_pos_itinerario?>
										</a>
									</div>

									<div id="itinerario<?=$ord_itinerario?>" class="accordion-body collapse">
							...
							<table class="span7 table-hover">
								...
								<tbody>
									...
									?>
										<td><?=$fec_salida?></td>
										<td>
											<?php if ($porc>=0.3 && $cant!=1 && $cant!=0) { ?><a class="tooltip-test" title="Disponible - <?=$des_salida?>"><img class="ball verde"></img></a> <?php } ?> 
											<?php if ($porc<0.3 && $cant!=1 && $cant!=0) { ?><a class="tooltip-test" title="Pocos Lugares - <?=$des_salida?>"><img class="ball amarillo"></img></a> <?php } ?> 
											<?php if ($cant==1) { ?><a class="tooltip-test" title="Solo un Lugar - <?=$des_salida?>"><img class="ball ambar"></img></a> <?php } ?> 
											<?php if ($cant==0 && $pet_salida!=1) { ?><a class="tooltip-test" title="Agotado - <?=$des_salida?>"><img class="ball rojo"></img></a> <?php } ?> 
											<?php if ($cant==0 && $pet_salida==1) { ?><a class="tooltip-test" title="A peticion - <?=$des_salida?>"><img class="ball guinda"></img></a> <?php } ?> 
										</td>
									<?php 
