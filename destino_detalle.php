...
if (empty($region) AND empty($pais) AND empty($ciudad)) {
	//con la url encontrar el id
	$dato_region = "SELECT id_pagina FROM pagina WHERE url_pagina='$url_pagina' AND niv_pagina='2'";
	$dato_region = mysql_db_query($dbname, $dato_region); 
	if ($fila = mysql_fetch_array($dato_region)){ $id = $fila["id_pagina"]; }

} else{

	if (empty($pais) AND empty($ciudad)) {

		//con la url encontrar el id
		$dato_region = "SELECT id_pagina FROM pagina WHERE url_pagina='$region' AND niv_pagina='2'";
		$dato_region = mysql_db_query($dbname, $dato_region); 
		while ($fila = mysql_fetch_array($dato_region)){ 

			$id_pag_region = $fila["id_pagina"];

			//con la url encontrar el id
			$dato_pais = "SELECT id_pagina FROM pagina WHERE url_pagina='$url_pagina' AND niv_pagina='3' AND per_pagina='$id_pag_region'";
			$dato_pais = mysql_db_query($dbname, $dato_pais); 
			if ($filb = mysql_fetch_array($dato_pais)){ $id = $filb["id_pagina"]; }
		}
		...
		if (empty($ciudad)) {
			...
			while ($fila = mysql_fetch_array($dato_region)){ 
				...
				//con la url encontrar el id
				$dato_pais = "SELECT id_pagina FROM pagina WHERE url_pagina='$pais' AND niv_pagina='3' AND per_pagina='$id_pag_region'";
				$dato_pais = mysql_db_query($dbname, $dato_pais); 
				while ($filb = mysql_fetch_array($dato_pais)){ 

					$id_pag_pais = $filb["id_pagina"]; 

					//con la url encontrar el id
					$dato_ciudad = "SELECT id_pagina FROM pagina WHERE url_pagina='$url_pagina' AND niv_pagina='4' AND per_pagina='$id_pag_pais'";
					$dato_ciudad = mysql_db_query($dbname, $dato_ciudad); 
					if ($filc = mysql_fetch_array($dato_ciudad)){ $id = $filc["id_pagina"]; }
				}
			}
			...
			while ($fila = mysql_fetch_array($dato_region)){ 
				...
				while ($filb = mysql_fetch_array($dato_pais)){ 
					...
					//con la url encontrar el id
					$dato_ciudad = "SELECT id_pagina FROM pagina WHERE url_pagina='$ciudad' AND niv_pagina='4' AND per_pagina='$id_pag_pais'";
					$dato_ciudad = mysql_db_query($dbname, $dato_ciudad); 
					while ($filc = mysql_fetch_array($dato_ciudad)){ 

						$id_pag_ciudad = $filc["id_pagina"];

						//con la url encontrar el id
						$dato_interior = "SELECT id_pagina FROM pagina WHERE url_pagina='$url_pagina' AND niv_pagina='5' AND per_pagina='$id_pag_ciudad'";
						$dato_interior = mysql_db_query($dbname, $dato_interior); 
						if ($fild = mysql_fetch_array($dato_interior)){ $id = $fild["id_pagina"]; }			
				...
				<div class="span8">
					<article class="span8 text-center">
						<h1 class="inline-block pull-left">
							<a href="/destinos"><i class="icon-destino ico-titulo"></i></a>
						</h1>	
						<h1 class="inline-block pull-left">
							<a href="/destinos" class="titulo">Destinos</a> » 
						</h1>
						<br><br><br><br>
						<?php
							...
							if (!empty($ids_dest)) {
								...
								while ($row = mysql_fetch_array($des_pad)){ 
									...
									?>
										<div class="inline-block pull-left text-left titulo">
											<div class="dropdown">
												<a href="#" class="dropdown-toggle " data-toggle="dropdown">
													<h1> <?=$tit_des?><b class="caret"></b></h1>
												</a>
												<ul class="dropdown-menu">
													<?php
														//tipo
														$dato_tip_tour = "SELECT * FROM pagina WHERE per_pagina='$per_des'";
										...
										<h1 class="inline-block pull-left"> » </h1>
									<?php
						...
						<p>
							<?php
								...
								$dato_valoracion = "SELECT 
														...
														FROM
														((destino LEFT JOIN pagina ON pagina.id_pagina = destino.id_pagina)
						...
						if ($niv<5) {
							...
							<article class="span8">
								<div class="text-left span4 tour-tipo">
									<h5 class="text-left"><i class="icon-destino"></i> Destinos Internos »</a></h5>
									<ul class="text-left">
										<?php

											$dato_pag_tours = "SELECT id_pagina,tit_pos_pagina FROM pagina WHERE per_pagina=$id";
											$dato_pag_tours = mysql_db_query($dbname, $dato_pag_tours); 
											while ($row = mysql_fetch_array($dato_pag_tours)){ 

												$id_pag_dest_int = $row["id_pagina"];
												$tit_pag_dest_int = $row["tit_pos_pagina"];

												$url_pag = pagUrl($dbname,$id_pag_dest_int);$pagurl="";
												$url_pag_dest_int = substr($url_pag, 0, -1);

												?>
												<li>
													<a href="/<?=$url_pag_dest_int?>"><?=$tit_pag_dest_int?></a>
												</li>
											<?php
											}
										?>
