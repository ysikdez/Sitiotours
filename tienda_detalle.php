...
$dato_pagina = "SELECT * FROM (pagina LEFT JOIN tienda ON pagina.id_pagina = tienda.id_pagina) WHERE pagina.id_pagina='$id'";
...
//Tienda
	...
	$open_a_tienda = strtotime($open_a_tienda);
	$open_a_tienda = date("h:i a", $open_a_tienda);

	$close_a_tienda = $row["close_a_tienda"];
	$close_a_tienda = strtotime($close_a_tienda);
	$close_a_tienda = date("h:i a", $close_a_tienda);

	$open_b_tienda = $row["open_b_tienda"];
	$open_b_tienda = strtotime($open_b_tienda);
	$open_b_tienda = date("h:i a", $open_b_tienda);

	$close_b_tienda = $row["close_b_tienda"];
	$close_b_tienda = strtotime($close_b_tienda);
	$close_b_tienda = date("h:i a", $close_b_tienda);	
					...
					<article class="span8 text-center">
						...
						<small>
							<dl class="dl-horizontal text-left">
								...
								<dd>
									<?php
										...
										$dato_destino = "SELECT pagina.id_pagina ,destino.id_destino ,pagina.tit_pos_pagina ,pagina.url_pagina
																FROM
																((tienda LEFT JOIN destino ON tienda.id_destino = destino.id_destino)
								...
								<dt><strong>Horario :</strong></dt>
								<dd>
									<?=$open_a_tienda?> - <?=$close_a_tienda?><br>
									<?php
										if ($open_b_tienda<>"12:00 am" && $close_b_tienda<>"12:00 am") {
											?>
												<?=$open_b_tienda?> - <?=$close_b_tienda?>
											<?php
										}
									?>
								</dd>

								<dt><strong>Aforo :</strong></dt>
								<dd><?=$afo_tienda?> personas</dd>

							</dl>
						</small>
						<br>
						<h4 class="text-left">Descripcion :</h4>
						<p class="text-justify"><?=$des_tienda?></p>
						<br><br>
						<p>
							<?php
						...
						<?php
							...
							//Datos de las Platos
							$datos_platos = "SELECT id_articulo,tit_articulo,des_articulo,car_articulo FROM articulo WHERE id_tienda='$id_tienda' ORDER BY ord_articulo";
						...
						?>
							<h4 class="text-left"><?=$tit_articulo?></h4>
							<h5 class="text-left">Descripcion</h5>
							<p class="text-justify"><?=$des_articulo?></p>
							<h5 class="text-left">Caracteristicas</h5>
							<p class="text-justify"><?=$car_articulo?></p>
							...
							<small>
								<dl class="dl-horizontal text-left">
									...
									<dd>
										...
										$datos_pre_plato = "SELECT nom_categoria,des_categoria
																FROM categoria
																LEFT JOIN articulo_categoria ON articulo_categoria.id_categoria = categoria.id_categoria
																WHERE
																articulo_categoria.id_articulo='$id_articulo'";
										...
										?>
											<a class="tooltip-test" data-placement="right" title="<?=$des_categoria?>"><?=$nom_categoria?></a>,
									...
									<dt><strong>Ocasion :</strong></dt>
									<dd>
										<?php

										//Datos de las precios
										$datos_ocasion = "SELECT nom_ocasion,des_ocasion
																FROM ocasion
																LEFT JOIN articulo_ocasion ON articulo_ocasion.id_ocasion = ocasion.id_ocasion
																WHERE
																articulo_ocasion.id_articulo='$id_articulo'";
										$datos_ocasion = mysql_db_query($dbname, $datos_ocasion); 
										while ($row = mysql_fetch_array($datos_ocasion)){ 

											$nom_ocasion = $row["nom_ocasion"];
											$des_ocasion = $row["des_ocasion"];
										?>
											<a class="tooltip-test" data-placement="right" title="<?=$des_ocasion?>"><?=$nom_ocasion?></a>,
									...
									<dt><strong>Dirigido Para :</strong></dt>
									<dd>
										<?php

										//Datos para quien es?
										$datos_dirigido = "SELECT nom_dirigido,des_dirigido
																FROM dirigido
																LEFT JOIN articulo_dirigido ON articulo_dirigido.id_dirigido = dirigido.id_dirigido
																WHERE
																articulo_dirigido.id_articulo='$id_articulo'";
										$datos_dirigido = mysql_db_query($dbname, $datos_dirigido); 
										while ($row = mysql_fetch_array($datos_dirigido)){ 

											$nom_dirigido = $row["nom_dirigido"];
											$des_dirigido = $row["des_dirigido"];
										?>
											<a class="tooltip-test" data-placement="right" title="<?=$des_dirigido?>"><?=$nom_dirigido?></a>,
									...
									<dd>
										...
										//Datos de las precios
										$datos_pre_plato = "SELECT * FROM aprecio WHERE id_articulo='$id_articulo' AND ofe_aprecio='0' ORDER BY ord_aprecio";
										$datos_pre_plato = mysql_db_query($dbname, $datos_pre_plato); 
										while ($row = mysql_fetch_array($datos_pre_plato)){ 
										...
										?>
											<a class="tooltip-test" data-placement="right" title="<?=$ini_aprecio?> - <?=$fin_aprecio?> : <?=$des_aprecio?>"><?=$uni_aprecio?> x <?=$val_aprecio?> USD</a><br>
									...
									<dt><strong>Ofertas :</strong></dt>
									<dd>
										<?php

										//Datos de las precios
										$datos_pre_plato = "SELECT * FROM aprecio WHERE id_articulo='$id_articulo' AND ofe_aprecio='1' ORDER BY ord_aprecio";
										$datos_pre_plato = mysql_db_query($dbname, $datos_pre_plato); 
										while ($row = mysql_fetch_array($datos_pre_plato)){ 
