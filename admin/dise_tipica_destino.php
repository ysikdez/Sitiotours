...
//Datos del tipica
$pos_tour = "SELECT * FROM tipica WHERE id_pagina='$ids'";
...
if (!$_POST) {
	...
	if ($region=="Ninguna") {
		...
		//Nuevo destino
		$nuevo_destino = "UPDATE tipica SET id_destino = $id_new_des WHERE id_pagina='$ids'";
	...
	header("location: dise_tipica_destino.php?id=$ids&error=$error");
}
...
<html lang="es">
	...
	<body>
		<div class="row contorno">
			<a class="btn btn-sitio pull-right" href="dise_tipica.php?id=<?=$ids?>"><i class="icon-arrow-left"></i></a>
			<h3>Destino »</h3>  
