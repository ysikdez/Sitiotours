...
$datos_sitio = "SELECT * FROM sitio WHERE id_pagina='$ids'";
$datos_sitio = mysql_db_query($dbname, $datos_sitio); 
if ($row = mysql_fetch_array($datos_sitio)){ 

	$id_sitio = $row["id_sitio"];
	$cod_sitio = $row["cod_sitio"];
	$des_sitio = $row["des_sitio"];
...
if (!$_POST) {
	...
	if ($ima_def==1) {
		...
		$dise_sitio = "UPDATE sitio SET 

							des_sitio = '$des'

						WHERE id_pagina='$ids'";

		$db_dise_sitio = mysql_db_query($dbname, $dise_sitio);
	...
	header("location: dise_sitio.php?id=$ids&error=$error");
}
...
<html lang="es">
	<head>
		...
		<title>Editar Sitio Turistico</title>
	</head>
	<body onload="imaPrincipal();">
		<div class="row contorno">
			<form method="post" enctype="multipart/form-data" name="formulario" action="">
				...
				<br><br>
				<h3>Editar Sitio Turisticos » </h3>						
				<h4>» <?=$tit_pos_pag_pad?></h4>
				<h5>» <?=$tit_pos_pag?></h5>
				<br>
				<h5>Codigo del Sitio Turistico » <?=$cod_sitio?></h5>
				<br>
				<hr>
				<h5>Destino »
					<a class="btn btn-sitio pull-right" href="dise_sitio_destino.php?id=<?=$ids?>"><i class="icon-arrow-right"></i></a>
				</h5>
				<hr>
				<h5>Galeria: Fotos »
				...
				<h5>Descripción »</h5>
				<textarea class="ckeditor span5" name="des"><?=$des_sitio?></textarea>
				<br>				
				...
				<?php if (!empty($logo_pag)) { ?>
					<div class="text-center">
						<div class="tour-caja ico-prin">
							<a href="dise_sitio_logo_eli.php?id=<?=$ids?>"><div class="pull-right icono"><i class="icon-remove"></i></div></a>
