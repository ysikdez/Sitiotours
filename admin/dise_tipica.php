...
$datos_tipica = "SELECT * FROM tipica WHERE id_pagina='$ids'";
$datos_tipica = mysql_db_query($dbname, $datos_tipica); 
if ($row = mysql_fetch_array($datos_tipica)){ 

	$id_tipica = $row["id_tipica"];
	$cod_tipica = $row["cod_tipica"];
	$des_tipica = $row["des_tipica"];
	$ing_tipica = $row["ing_tipica"];
	$ela_tipica = $row["ela_tipica"];
...
if (!$_POST) {
	...
	if ($ima_def==1) {
		...
		$db_dise_tipica = mysql_db_query($dbname, $dise_tipica);
	...
	header("location: dise_tipica.php?id=$ids&error=$error");
}
...
<html lang="es">
	<head>
		...
		<title>Editar Comida Tipica</title>
	</head>
	<body onload="imaPrincipal();">
		<div class="row contorno">
			<form method="post" enctype="multipart/form-data" name="formulario" action="">
				...
				<br><br>
				<h3>Editar Datos de la Comida Tipica » </h3>						
				<h4>» <?=$tit_pos_pag_pad?></h4>
				<h5>» <?=$tit_pos_pag?></h5>
				<br>
				<h5>Codigo de la Comida Tipica » <?=$cod_tipica?></h5>
				<br>
				<hr>
				<h5>Destino »
					<a class="btn btn-sitio pull-right" href="dise_tipica_destino.php?id=<?=$ids?>"><i class="icon-arrow-right"></i></a>
				</h5>
				<hr>
				<h5>Galeria: Fotos »
				...
				<h5>Descripción »</h5>
				<textarea class="ckeditor span5" name="des"><?=$des_tipica?></textarea>
				<br>				
				
				<h5>Materiales »</h5>
				<textarea class="ckeditor span5" name="ing"><?=$ing_tipica?></textarea>
				<br>				
				
				<h5>Elaboración »</h5>
				<textarea class="ckeditor span5" name="ela"><?=$ela_tipica?></textarea>
				<br>
				<label>
				...
				<?php if (!empty($logo_pag)) { ?>
					<div class="text-center">
						<div class="tour-caja ico-prin">
							<a href="dise_tipica_logo_eli.php?id=<?=$ids?>"><div class="pull-right icono"><i class="icon-remove"></i></div></a>
