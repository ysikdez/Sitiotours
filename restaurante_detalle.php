...
$dato_pagina = "SELECT * FROM (pagina LEFT JOIN restaurante ON pagina.id_pagina = restaurante.id_pagina) WHERE pagina.id_pagina='$id'";
...
//Restaurante
	...
	$open_a_restaurante = strtotime($open_a_restaurante);
	$open_a_restaurante = date("h:i a", $open_a_restaurante);

	$close_a_restaurante = $row["close_a_restaurante"];
	$close_a_restaurante = strtotime($close_a_restaurante);
	$close_a_restaurante = date("h:i a", $close_a_restaurante);

	$open_b_restaurante = $row["open_b_restaurante"];
	$open_b_restaurante = strtotime($open_b_restaurante);
	$open_b_restaurante = date("h:i a", $open_b_restaurante);

	$close_b_restaurante = $row["close_b_restaurante"];
	$close_b_restaurante = strtotime($close_b_restaurante);
	$close_b_restaurante = date("h:i a", $close_b_restaurante);	
					...
					<article class="span8 text-center">
						...
						<small>
							<dl class="dl-horizontal text-left">
								...
								<dd>
									<?php
										...
										$dato_destino = "SELECT pagina.id_pagina ,destino.id_destino ,pagina.tit_pos_pagina ,pagina.url_pagina
																FROM
																((restaurante LEFT JOIN destino ON restaurante.id_destino = destino.id_destino)
								...
								<dd>
									<?=$open_a_restaurante?> - <?=$close_a_restaurante?><br>
									<?php
										if ($open_b_restaurante<>"12:00 am" && $close_b_restaurante<>"12:00 am") {
											?>
												<?=$open_b_restaurante?> - <?=$close_b_restaurante?>
											<?php
										}
									?>
								</dd>
								...
								<dd><?=$cat_restaurante?> Tenedores</dd>

								<dt><strong>Aforo :</strong></dt>
								<dd><?=$afo_restaurante?> personas</dd>

								<dt><strong>Especialidad :</strong></dt>
								<dd>
									<?php
										//Especialidad
										$dato_especialidad = "SELECT DISTINCT nom_especialidad,des_especialidad
															FROM
															especialidad
															LEFT JOIN restaurante_especialidad ON restaurante_especialidad.id_especialidad = especialidad.id_especialidad
															WHERE
															restaurante_especialidad.id_restaurante='$id_restaurante'";
										...
										while ($row = mysql_fetch_array($dato_especialidad)){ 
											...
											?>
												<a class="tooltip-test" title="<?=$des_especialidad?>"><?=$nom_especialidad?></a>, 
											<?php
										}
									?>
									<br>
								...
								<dd>
									<?php
										//Tipo
										$dato_tipo = "SELECT DISTINCT nom_tipo,des_tipo
															FROM
															tipo
															LEFT JOIN restaurante_tipo ON restaurante_tipo.id_tipo = tipo.id_tipo
															WHERE
															restaurante_tipo.id_restaurante='$id_restaurante'";
										...
										while ($row = mysql_fetch_array($dato_tipo)){ 
											...
											?>
												<a class="tooltip-test" title="<?=$des_tipo?>"><?=$nom_tipo?></a>, 
											<?php
										}
									?>
									<br>
								...
								<dd>
									<?php
										//Ocasion
										$dato_rocasion = "SELECT DISTINCT nom_rocasion,des_rocasion
															FROM
															rocasion
															LEFT JOIN restaurante_rocasion ON restaurante_rocasion.id_rocasion = rocasion.id_rocasion
															WHERE
															restaurante_rocasion.id_restaurante='$id_restaurante'";

										$dato_rocasion = mysql_db_query($dbname, $dato_rocasion); 
										while ($row = mysql_fetch_array($dato_rocasion)){ 

											$nom_rocasion = $row["nom_rocasion"];
											$des_rocasion = $row["des_rocasion"];
											
											?>
												<a class="tooltip-test" title="<?=$des_rocasion?>"><?=$nom_rocasion?></a>, 
											<?php
										}
									?>
									<br>
						...
						<p class="text-justify"><?=$des_restaurante?></p>
						<?php
						...
						?>
							<h4 class="text-left"><?=$nom_plato?></h4>
							<h5 class="text-left">Descripcion</h5>
							<p class="text-justify"><?=$des_plato?></p>
							<h5 class="text-left">Ingredientes</h5>
							<p class="text-justify"><?=$ing_plato?></p>
							<h5 class="text-left">Elaboracion</h5>
							<p class="text-justify"><?=$ela_plato?></p>
							<small>
								<dl class="dl-horizontal text-left">
									...
									<dd>
										...
										$datos_pre_plato = "SELECT * FROM pprecio WHERE id_plato='$id_plato' AND ofe_pprecio='0' ORDER BY ord_pprecio";
										...
										?>
											<a class="tooltip-test" data-placement="right" title="<?=$ini_pprecio?> - <?=$fin_pprecio?> : <?=$des_pprecio?>"><?=$pla_pprecio?> x <?=$val_pprecio?> USD</a><br>
									...
									<dd>
										...
										//Datos de las precios
										$datos_pre_plato = "SELECT * FROM pprecio WHERE id_plato='$id_plato' AND ofe_pprecio='1' ORDER BY ord_pprecio";
										$datos_pre_plato = mysql_db_query($dbname, $datos_pre_plato); 
										while ($row = mysql_fetch_array($datos_pre_plato)){ 
